...
class LMS:
    ...
    def Issue_books(self):
        books_ids = input("Enter Books IDs (separated by comma) : ").split(',')
        current_date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        for books_id in books_ids:
            books_id = books_id.strip()
            if books_id in self.books_dict.keys():
                if not self.books_dict[books_id]['status'] == 'Available':
                    ...
                    your_name = input(f"Enter Your Name for Book ID {books_id}: ")
                    self.books_dict[books_id]['lender_name'] = your_name
                    self.books_dict[books_id]['lend_date'] = current_date
                    self.books_dict[books_id]['status']= 'Already Issued'
                    print(f"Book {books_id} Issued Successfully !!!\n")
            else:
                print(f"Book ID {books_id} Not Found !!!")

    def add_books(self):
        new_books = input("Enter Books Title : ")
        if new_books == "":
            return self.add_books()
        elif len(new_books) > 40:
            print("Books title length is too long !!! Title length limit is 40 characters")
            return self.add_books()
        else:
        ...
        for books_id in books_ids:
            ...
            if books_id in self.books_dict.keys():
                if self.books_dict[books_id]['status'] == 'Available':
                    print(f"Book with ID {books_id} is already available in library. Please check book id.")
                else:
                    self.books_dict[books_id]['lender_name'] = ''
                    self.books_dict[books_id]['lend_date'] = ''
                    self.books_dict[books_id]['status'] = 'Available'
                    print(f"Successfully Updated the status of book with ID {books_id}!!!\n")
            else:
                print(f"Book ID {books_id} Not Found !!!")
